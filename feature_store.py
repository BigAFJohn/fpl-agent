...
3. The join strategy
   player_features × fixture_difficulty: join on team_id + gameweek
   player_features × lineup_probability: join on player_id + gameweek
...
def setup_model_features_table(engine):
    """
    Creates model_features — the final consolidated feature store.
    This is the direct input to Phase 4's XGBoost model.
    Dropped and rebuilt on each run.
    """
...
def build_model_features(engine):
    """
    Joins player_features + fixture_difficulty + lineup_probability
    into the consolidated model_features table.
    """
    print("\n[1/3] Loading source tables...")

    # Load player features with team name and was_home
    pf = pd.read_sql("""
        SELECT pf.*,
               ph.was_home,
               t.name AS team_name
    ...
    # Load lineup probability — only columns that exist in the table
    lp = pd.read_sql("""
    ...
    def get_fix(team_name, gameweek, col):
        """Get a fixture column value for a team+gameweek."""
    ...
    for _, row in pf.iterrows():
        ...
        if season == "2025-26" and team_name:
            fix_fdr  = get_fix(team_name, gameweek, "fixture_fdr")
            opp_xga  = get_fix(team_name, gameweek, "opponent_xga_5")
            fpl_fdr  = get_fix(team_name, gameweek, "fpl_fdr")
            opp_name = get_fix(team_name, gameweek, "opponent_name")

        # Lineup probability — current GW only
        lp_prob   = None
        lp_status = None
        lp_chance = None
        lp_conf   = None
        # Injury data — prefer lineup_probability table, fall back to player_features
        ...
        if lp_key in lp_map.index:
            ...
            # Override injury score from lineup table if available
...
def verify_model_features(engine):
    """
    Final sanity checks on the consolidated feature store.
    Shows feature coverage and top predicted picks for next gameweek.
    """
    ...
    # Top picks — form × lineup_prob × fixture ease
    print("\n  Top GW picks (form × lineup_prob × fixture ease):")
...
def run_feature_store():
    """
    Consolidates all Phase 3 features into model_features table.
    1. Joins player_features + fixture_difficulty + lineup_probability
    2. Selects home/away FDR based on each player's fixture
    3. Writes consolidated model_features table
    4. Verifies coverage and shows top picks
    """
    start = datetime.now()
